#!/usr/bin/env python3
"""
Process Crawled Data - Xử lý và tích hợp dữ liệu đã crawl vào training
"""
...
class CrawledDataProcessor:
    ...
    def load_crawled_data(self, csv_path: str = "data/crawled_reviews.csv") -> pd.DataFrame:
        """Load dữ liệu đã crawl từ CSV file"""
        print("🔄 Loading crawled data from CSV...")
        ...
        try:
            ...
            # Load từ CSV
            df_crawled = pd.read_csv(csv_path, encoding='utf-8')
            
            print(f"✅ Loaded {len(df_crawled)} crawled reviews from CSV")
            return df_crawled
    ...
    def preprocess_crawled_text(self, text: str) -> str:
        """Preprocess text từ crawled data"""
    ...
    def filter_quality_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Preprocess text
        df['processed_text'] = df['text'].apply(self.preprocess_crawled_text)
    ...
    def balance_data(self, df: pd.DataFrame) -> pd.DataFrame:
        """Cân bằng dữ liệu theo sentiment"""
        print("⚖️ Balancing data by sentiment...")
    ...
    def integrate_with_training_data(self, df_crawled: pd.DataFrame) -> None:
        """Tích hợp dữ liệu đã crawl vào training data"""
        print("🔄 Integrating crawled data with training data...")
        ...
        try:
            ...
            for _, row in df_crawled.iterrows():
                ...
                rows.append((
                    ...
                    row['content'],         # original_text
                    LABEL_MAP[row['validated_sentiment']],  # label
                    f"crawled_{row['source']}",  # source_file
                    1.0,                    # confidence
            ...
            print(f"✅ Integrated {len(rows)} reviews into training data")
            
        except Exception as e:
            print(f"❌ Error integrating data: {e}")
    
    def generate_synthetic_neutral_data(self, count: int = 1000) -> pd.DataFrame:
        """Tạo dữ liệu neutral synthetic"""
        print(f"🔄 Generating {count} synthetic neutral samples...")
        ...
        for i in range(count):
            ...
            # Preprocess
            processed_text = self.preprocess_crawled_text(text)
        ...
        print(f"✅ Generated {len(df_synthetic)} synthetic neutral samples")
        
        return df_synthetic
    
    def process_all_data(self, csv_path: str = "data/crawled_reviews.csv"):
        """Xử lý toàn bộ dữ liệu"""
        print("🚀 Starting data processing...")
        print("=" * 60)
        
        # 1. Load crawled data từ CSV
        df_crawled = self.load_crawled_data(csv_path)
        
        if len(df_crawled) == 0:
            print("⚠️ No crawled data found. Generating synthetic data...")
            df_crawled = self.generate_synthetic_neutral_data(1000)
        else:
            # 2. Filter quality data
            df_crawled = self.filter_quality_data(df_crawled)
            
            # 3. Balance data
            df_crawled = self.balance_data(df_crawled)
        
        # 4. Generate synthetic neutral data
        df_synthetic = self.generate_synthetic_neutral_data(500)
        
        # 5. Combine data
        df_combined = pd.concat([df_crawled, df_synthetic], ignore_index=True)
